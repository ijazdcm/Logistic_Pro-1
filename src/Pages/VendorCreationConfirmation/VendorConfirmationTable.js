...
const VendorCreationConfirmationTable = () => {
  ...
  const loadVendorApprovalTable = () => {
    VendorCreationService.getVendorConfirmationTableData().then((res) => {
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`VendorCreationConfirmation/${data.vehicle_id}`}>
                Confirm Vendor
              </Link>
            </CButton>
          ),
...
export default VendorCreationConfirmationTable
